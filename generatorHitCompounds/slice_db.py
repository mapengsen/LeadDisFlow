...
class SliceDB(ma.Action):
    ...
    def run(self):
        def _enumerate(row, max_cuts=self.max_cuts, enumerator=self.enumerator):
            fields = row.split("\t")
            # 这里的smiles就是待slice的分子：O=C(N[C@@H](C)C1=CC=C(C(O)=O)C=C1)C2=C(CC3=CC=C(OC(F)F)C(OC(F)F)=C3)SC4=C2CCOC4
            smiles = fields[0]
...
def parse_args():
    ...
    parser.add_argument("--input-smiles-path", "-i", help="Path to the input file with molecules in SMILES notation.", type=str, required=True)
    parser.add_argument("--output-parquet-folder", "-o", help="Path to the output Apache Parquet folder.", type=str)
    parser.add_argument("--output-smiles-path", "-u", help="Path to the output SMILES file.", type=str)
    ...
    if args.output_smiles_path:
        with open(args.output_smiles_path, "w+") as smiles_file:
            for row in slice_df.rdd.map(_to_smiles_rows).toLocalIterator():
                # row_list = row.split('\t')
                # if get_scaffold(row_list[0]) == get_scaffold(row_list[2]):
                #     smiles_file.write("{}\n".format(row))
                smiles_file.write("{}\n".format(row))
                ...
                # # The silce rule in Train Model
                # if args.trainModel:
                #     smiles_file.write("{}\n".format(row))
                # # The silce rule in Inference Model
                # else:
                #     row_list = row.split('\t')
                #     if get_scaffold(row_list[0]) == get_scaffold(row_list[2]):
                #         smiles_file.write("{}\n".format(row))
