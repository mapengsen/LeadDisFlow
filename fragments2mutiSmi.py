...
def parse_args():
    ...
    parser.add_argument("--input-smi-path", "-i", help="Path to a SMILES file to convert with scaffolds and decorations.", type=str, required=True)
    parser.add_argument("--output-smi-folder-path", "-o", help="Path to a folder that will have the converted SMILES files.", type=str, required=True)
    parser.add_argument("--num-files", "-n", help="Number of SMILES files to create (numbered from 000 ...) [DEFAULT: 1]", type=int, default=1)
    parser.add_argument("--num-partitions", "-p", help="Number of SPARK partitions to use [DEFAULT: 1000]", type=int, default=1000)
    parser.add_argument("--decorator-type", "-d", help="Type of training set, depending on the decorator TYPES=(single, multi) [DEFAULT: multi].", type=str, default="multi")
